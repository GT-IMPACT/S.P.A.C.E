...
class MapGenCaveMars : MapGenMetaBase() {
    protected fun generateLargeCaveNode(
        ...
        par6: Double, par8: Double, par10: Double
    ) {
        generateCaveNode(
            par1, par3, par4, blockIdArray, metaArray,
            par6, par8, par10, 
    ...
    protected fun generateCaveNode(
        par1: Long, par3: Int, par4: Int, blockIdArray: Array<Block?>, metaArray: ByteArray, par6: Double,
        par8: Double, par10: Double, par12: Float, par13: Float, par14: Float, par15: Int, par16: Int, par17: Double
    ) {
        var par6 = par6
        var par8 = par8
        var par10 = par10
        ...
        while (par15 < par16) {
            val d6 = 1.5 + MathHelper.sin(par15 * Math.PI.toFloat() / par16) * par12 * 1.0f
            val d7 = d6 * par17
            ...
            if (!flag && par15 == k1 && par12 > 1.0f && par16 > 0) {
                generateCaveNode(
                    random.nextLong(), par3, par4, blockIdArray, metaArray,
                    par6, par8, par10,
                    random.nextFloat() * 0.5f + 0.5f, par13 - Math.PI.toFloat() / 2f,
                    par14 / 3.0f, par15, par16, 1.0
                )
                generateCaveNode(
                    random.nextLong(), par3, par4, blockIdArray, metaArray, 
                    par6, par8, par10, 
    ...
    companion object {
        const val BREAK_THROUGH_CHANCE = 25
    }
}